...
def make_item(id: int, source: str, topics: List[str] = None, score: float = 1.0) -> Dict[str, Any]:
...
def make_items_from_sources(sources: List[str]) -> List[Dict[str, Any]]:
    ...
    return [make_item(id=i, source=src) for i, src in enumerate(sources, start=1)]
...
class TestNoConsecutiveSameSource:
    ...
    def test_three_sources(self):
        ...
        items = make_items_from_sources(["Verge", "Verge", "Verge", "TC", "TC", "Wired", "Wired"])
        ...
        for i in range(len(sources) - 1):
            assert sources[i] != sources[i + 1], f"Consecutive same source: {sources}"
...
class TestWindowSourceCap:
    ...
    def test_window_cap_enforced(self):
        ...
        for i in range(len(sources) - 4):
            ...
            for src in set(window):
                ...
                assert count <= 2, f"Source {src} appears {count} times in window {i}: {window}"
    ...
    def test_stricter_cap_for_reels(self):
        ...
        for i in range(len(sources) - 3):
            ...
            for src in set(window):
                count = window.count(src)
                assert count <= 1, f"Source {src} appears {count} times in window {i}: {window}"
    ...
    def test_topic_cap_enforced(self):
        ...
        for i in range(len(topics) - 4):
            ...
            for topic in set(window):
                ...
                assert count <= 2, f"Topic {topic} appears {count} times in window {i}: {window}"

    def test_no_topic_cap_when_none(self):
        """Should not enforce topic cap when set to None."""
        items = [make_item(i, chr(65 + i), topics=["AI"]) for i in range(10)]
...
class TestConstraintRelaxation:
    ...
    def test_progressive_relaxation(self):
        ...
        items = [make_item(i, "A") for i in range(1, 9)] + [make_item(i, "B") for i in range(9, 11)]
    ...
    def test_no_relaxation_when_not_needed(self):
        ...
        items = make_items_from_sources(["A", "B", "C", "D", "A", "B", "C", "D"])
...
class TestSurfaceConfiguration:
    ...
    def test_reels_stricter_than_articles(self):
        ...
        assert (
            reel_mixer.constraints.max_source_per_window
    ...
    def test_mix_feed_convenience_function(self):
        ...
        for i in range(len(sources) - 1):
            assert sources[i] != sources[i + 1], f"Consecutive same source at {i}: {sources}"
...
class TestSourceDistribution:
    ...
    def test_source_distribution_accuracy(self):
        ...
        items = make_items_from_sources(["A", "B", "A", "C", "B", "A"])
