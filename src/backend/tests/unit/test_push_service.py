...
def test_send_auto_deduplicates_ids_and_skips_non_eligible(monkeypatch):
    eligible = SimpleNamespace(
        ...
        image_url="https://cdn.example.com/eligible.jpg",
    ...
    candidate = SimpleNamespace(
        ...
        image_url="https://cdn.example.com/candidate.jpg",
        article_image_status="VERIFIED",
        ai_processed=False,
        summary=None,
        promotion_reason=None,
...
def test_evaluate_push_eligibility_rejects_thin_promoted_article():
    thin_article = SimpleNamespace(
        ...
        image_url="https://cdn.example.com/thin.jpg",
        article_image_status="VERIFIED",
        ai_processed=False,
        summary=None,
        promotion_reason=None,
...
def test_notification_uses_app_name_for_title():
    ...
    ready_article = SimpleNamespace(
        ...
        image_url="https://cdn.example.com/title-only.jpg",
        article_image_status="VERIFIED",
        ai_processed=True,
    ...
    assert service._notification_title_for_item(ready_article) == "Blips News"


def test_notification_body_uses_item_title():
    service = PushNotificationService(
        db=MagicMock(),
        config_service=_FakeConfigService(
            PushRuntimeConfig(enabled=True, mode=PushMode.manual, config_ttl_seconds=300),
    ...
    assert service._notification_body_for_item(ready_article) == "Title only"
