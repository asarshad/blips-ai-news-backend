...
def test_run_topup_runs_video_discovery(monkeypatch):
    ...
    monkeypatch.setattr(topup_service, "_run_topup_followups", lambda: None)
...
def test_check_and_trigger_topup_uses_article_target_pending_even_when_health_is_ok(monkeypatch):
    db = object()
    db_factory = lambda: db
    triggered = []
    health = InventoryHealth(timestamp=datetime.utcnow(), surfaces={}, needs_topup=False)

    monkeypatch.setattr(topup_service, "_background_topup_enabled_for_process", lambda: True)
    monkeypatch.setattr(topup_service, "get_cached_inventory_health", lambda *_a, **_k: health)
    monkeypatch.setattr(topup_service, "_article_target_pending", lambda *_a, **_k: True)
    monkeypatch.setattr(
        topup_service,
        "trigger_topup_async",
        lambda passed_factory, priority_surfaces=None: triggered.append(
            (passed_factory, priority_surfaces)
        ),
    )

    result = topup_service.check_and_trigger_topup(db, db_factory)

    assert result is True
    assert triggered == [(db_factory, [])]


def test_run_topup_followups_only_runs_clustering(monkeypatch):
    """Promotion and AI are event-driven; top-up followup should only run clustering."""
    clustering_calls = []
    promotion_calls = []
