...
def upgrade() -> None:
    ...
    # Check and create contenttype enum
    result = conn.execute(sa.text("SELECT 1 FROM pg_type WHERE typname = 'contenttype'"))
    if not result.fetchone():
        op.execute("CREATE TYPE contenttype AS ENUM ('ARTICLE', 'VIDEO', 'REEL')")

    # Check and create preftype enum
    result = conn.execute(sa.text("SELECT 1 FROM pg_type WHERE typname = 'preftype'"))
    if not result.fetchone():
        op.execute("CREATE TYPE preftype AS ENUM ('TOPIC', 'ENTITY', 'SOURCE', 'FORMAT')")

    # Check and create eventtype enum
    result = conn.execute(sa.text("SELECT 1 FROM pg_type WHERE typname = 'eventtype'"))
    if not result.fetchone():
        op.execute(
            "CREATE TYPE eventtype AS ENUM ('VIEW_10S', 'OPEN_SOURCE', 'SHARE', 'SAVE', 'CHAT_START', 'CHAT_MESSAGE')"
        )

    # Check if content_items table already exists
    if "content_items" in inspector.get_table_names():
        ...
        contenttype_enum = postgresql.ENUM(
            "ARTICLE", "VIDEO", "REEL", name="contenttype", create_type=False
        )
        ...
        op.create_index(
            "ix_content_type_published_score",
            "content_items",
            ["type", "published_at", "global_score"],
        )
    ...
    # Create content_clusters table (if not exists)
    ...
    if "user_preferences" not in inspector.get_table_names():
        preftype_enum = postgresql.ENUM(
            "TOPIC", "ENTITY", "SOURCE", "FORMAT", name="preftype", create_type=False
        )
        ...
        op.create_unique_constraint(
            "uq_user_pref", "user_preferences", ["device_id", "pref_type", "key"]
        )
    ...
    if "interaction_events" not in inspector.get_table_names():
        ...
        op.create_index(
            "ix_interaction_events_content_item_id", "interaction_events", ["content_item_id"]
        )
        op.create_index("ix_interaction_events_created_at", "interaction_events", ["created_at"])
        op.create_index(
            "ix_interaction_device_created", "interaction_events", ["device_id", "created_at"]
        )
        op.create_index(
            "ix_interaction_content_created",
            "interaction_events",
