    ...
    def __post_init__(self):
        ...
        if self.source_contribution_pct is None:
            ...
            self.source_contribution_pct = {
                src: round(cnt / total * 100, 1) for src, cnt in self.source_distribution.items()
            }
...
class DiversityMixer:
    ...
    def _build_active_constraints(
        self, constraints: DiversityConstraints, relaxation_level: int = 0
    ) -> Dict[str, Any]:
    ...
    def _check_window_topic_constraint(
        ...
        topic_count = sum(1 for item in window if self._get_primary_topic(item) == candidate_topic)
    ...
    def mix(
        self, candidates: List[Any], target_size: int, session_seed: Optional[str] = None
    ) -> MixerResult:
        """
        ...
        relaxation_level = 0
        active_constraints = self._build_active_constraints(self.constraints, relaxation_level)
    ...
    def _calculate_order_preservation(self, original: List[Any], reordered: List[Any]) -> float:
        """
...
def mix_feed(
    items: List[Any], surface: str, target_size: int, session_seed: Optional[str] = None
) -> List[Any]:
    """
