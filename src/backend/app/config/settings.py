...
class SchedulerSettings(BaseSettings):
    ...
    news_fetch_interval_hours: int = Field(default=3, description="Hours between news fetches")
    news_fetch_interval_minutes: int = Field(default=30, description="Minutes between news fetches")
    scoring_interval_minutes: int = Field(default=60, description="Minutes between scoring runs")
...
class QuotaSettings(BaseSettings):
    ...
    max_messages_per_article: int = Field(default=3, description="Max AI chat messages per article")
...
class CacheSettings(BaseSettings):
    ...
    article_cache_count: int = Field(default=5, description="Number of articles to pre-cache")
    playlist_cache_ttl_seconds: int = Field(default=300, description="Playlist cache TTL")
    session_snapshot_ttl_seconds: int = Field(default=3600, description="Session snapshot TTL")
