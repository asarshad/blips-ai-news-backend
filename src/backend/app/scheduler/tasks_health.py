...
def check_ingestion_health() -> None:
    ...
    try:
        ...
        most_recent = db.query(
            func.max(ContentItem.created_at)
        ).scalar()
        ...
        # Log current status
        logger.info(
            f"Last ingestion: {most_recent.isoformat()}, "
            f"hours ago: {hours_since_ingestion:.1f}"
        )
...
def get_ingestion_metrics() -> dict:
    ...
    try:
        ...
        articles_2h = db.query(func.count(ContentItem.id)).filter(
            ContentItem.type == ContentType.ARTICLE,
            ContentItem.created_at >= two_hours_ago,
        ).scalar() or 0
        
        videos_2h = db.query(func.count(ContentItem.id)).filter(
            ContentItem.type == ContentType.VIDEO,
            ContentItem.created_at >= two_hours_ago,
        ).scalar() or 0
        
        # Counts in last 24 hours
        articles_24h = db.query(func.count(ContentItem.id)).filter(
            ContentItem.type == ContentType.ARTICLE,
            ContentItem.created_at >= twenty_four_hours_ago,
        ).scalar() or 0
        
        videos_24h = db.query(func.count(ContentItem.id)).filter(
            ContentItem.type == ContentType.VIDEO,
            ContentItem.created_at >= twenty_four_hours_ago,
        ).scalar() or 0
