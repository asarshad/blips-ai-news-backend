...
def trigger_ingestion(
    ...
    This is for dev/testing. In production, ingestion runs via the worker.
    Limited to prevent accidental large runs.
    """
    import os
    ...
    if env == "prod":
        raise HTTPException(
            status_code=403,
            detail="Manual ingestion not allowed in production"
        )
    
    if not flags.is_enabled("ingestion"):
        raise HTTPException(
            status_code=503,
            detail="Ingestion feature is disabled"
        )
    ...
    result = {
        ...
        "articles_failed": 0,
        "summarization_enabled": flags.is_enabled("summarization")
    }
    ...
    try:
        ...
        for article_data in articles[:limit]:
            try:
                processed = summarizer.summarize_article(
                    article_data,
