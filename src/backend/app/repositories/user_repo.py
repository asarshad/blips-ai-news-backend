...
class UserProfileRepository(BaseRepository[UserProfile]):
    ...
    def update_timestamp(self, device_id: str) -> bool:
        ...
        result = self.db.query(UserProfile).filter(
            UserProfile.device_id == device_id
        ).update({
...
class UserPreferenceRepository(BaseRepository[UserPreference]):
    ...
    def get_preferences(
        self,
        device_id: str,
        pref_type: Optional[PrefType] = None,
        ...
        query = self.db.query(UserPreference).filter(
            ...
            UserPreference.weight >= min_weight
        )
...
class InteractionEventRepository(BaseRepository[InteractionEvent]):
    ...
    def get_content_events(
        ...
        return self.db.query(InteractionEvent).filter(
            ...
            InteractionEvent.created_at >= cutoff
        ).order_by(desc(InteractionEvent.created_at)).all()
    ...
    def count_user_chat_messages_today(
        self,
        device_id: str,
        content_item_id: int
    ) -> int:
    ...
    def get_interacted_content_ids(
        self,
        device_id: str,
        hours_back: int = 72
    ) -> List[int]:
    ...
    def get_engagement_stats(
        ...
        unique_users = self.db.query(
            func.count(func.distinct(InteractionEvent.device_id))
        ).filter(
            InteractionEvent.content_item_id == content_item_id,
            InteractionEvent.created_at >= cutoff
        ).scalar()
        ...
        # Count total events
        total_events = self.db.query(InteractionEvent).filter(
            InteractionEvent.content_item_id == content_item_id,
            InteractionEvent.created_at >= cutoff
        ).count()
