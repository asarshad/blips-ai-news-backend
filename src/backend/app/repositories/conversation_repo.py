...
class ConversationRepository(BaseRepository[Conversation]):
    ...
    def get_content_messages(self, content_item_id: int) -> List[Conversation]:
        """Get all messages for a content item conversation, ordered by timestamp."""
        return (
            self.db.query(Conversation)
            .filter(Conversation.content_item_id == content_item_id)
            .order_by(Conversation.timestamp)
            .all()
        )

    def get_recent_messages(self, content_item_id: int, limit: int = 50) -> List[Conversation]:
    ...
    def add_message(self, content_item_id: int, sender: str, message: str) -> Conversation:
        """Add a new message to a conversation."""
        conversation = Conversation(content_item_id=content_item_id, sender=sender, message=message)
